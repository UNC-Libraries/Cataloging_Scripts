...
# Set files
mrcfile = 'data/796input.mrc'
outfile = 'output/796data.txt'
...
@recs.each do |rec|
  ...
  # Look at each 796 field
  rec.each_by_tag('796') do |the796|
    ...
    alle = []
    # Then, look at each subfield in that 796...
    the796.each do |sf|
      # count $9s
      ct9 += 1 if sf.code == '9'
      if sf.code == 'e'
    ...
    # If there are no $es, send one line per 796 to output list
    @lines << [v001, vs796, flag, "", ""] if cte == 0
    # If there is 1 or more $e, send one line per $e to output list
    if cte > 0
      alle.each do |e|
        @lines << [v001, vs796, flag, e, ""]
      end
    end
  end
end
